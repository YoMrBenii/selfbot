...
async def bump():
    ...
    try:
        ...
        async for cmd in channel.slash_commands(): # Removed query="messages" to see everything
            found_any = True
            # This will print every command name and the Bot ID owning it
            print(f"DEBUG: Found '{cmd.name}' from Bot ID: {cmd.application_id}")
            
            if cmd.name == "messages" and cmd.application_id == 720351927581278219:
                print("Match found!")
                command = cmd
                break
        
        if not found_any:
            print("Zero commands were returned by Discord. Check your permissions!")
